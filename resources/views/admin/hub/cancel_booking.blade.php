                        ...
                        <thead>
                            <tr>
                                ...
                                <th> <input type="text" id="cancel_car_model" name="cancel_car_model"
                                        class="form-control" placeholder="Model" style="padding: 0%;"></th>
                                <th><input type="text" id="cancel_register_number" name="cancel_register_number"
                                        class="form-control" placeholder="Registration Number"></th>
                                <th>Address</th>
                                <th>User Details</th>
                                <th>D/L Number</th>
                                <th><input type="text" id="cancel_booking_id" name="cancel_booking_id"
                                        class="form-control" style="padding: 0%    ;" placeholder="Booking ID"></th>
                                <th>Reschedule</th>
                                <th>Security Dep</th>
                                <th>Amount</th>
                            </tr>
                        ...
                        <tbody>
                            @if (!empty($bookings) && $bookings->isNotEmpty())
                                @foreach ($bookings as $item)
                                    @php
                                        $booking_details = !empty($item->details[0])
                                            ? json_decode($item->details[0]->car_details)
                                            : [];
                                        $booking_payment_details = !empty($item->details[0])
                                            ? json_decode($item->details[0]->payment_details, true)
                                            : [];
                                        $car_model = !empty($booking_details->car_model)
                                            ? $booking_details->car_model
                                            : [];
                                        ...
                                        @if (in_array('hub_risk_status', getAdminPermissions()) || in_array('hub_risk_comments', getAdminPermissions()))
                                            <td>
                                                <button class="btn btn-warning open-risk-modal"
                                                    data-id="{{ $item->id }}"
                                                    data-commend="{{ json_encode($comments) }}">
                                                    <h3>i</h3>
                                                </button>
                                            </td>
                                        @endif
