    ...
    <section class="content-header">
        <div class="container-fluid my-2">
            <div class="row mb-2">
                ...
                @if (in_array('city_list_create', $permissions))
                    <div class="col-sm-6 text-right">
                        <button class="btn btn-primary mb-3" id="add_city">Add City</button>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="container-fluid">
            <div class="card">
                ...
                <div class="card-body table-responsive p-0">
                    <table id="city_table" class="table table-hover text-nowrap">
                        <thead>
                            <tr>
                                <th>ID</th>
                                <th>Name</th>
                                ...
                                @if (in_array('city_list_update', $permissions) || in_array('city_list_delete', $permissions))
                                    <th>Action</th>
                                @endif
                            </tr>
                        </thead>
                        <tbody>
                            @if (!empty(!$city->isEmpty()))
                                @foreach ($city as $item)
                                    <tr>
                                        ...
                                        @if (in_array('city_list_update', $permissions))
                                            <td>
                                                <a href="javascript:void(0)" class="city_edit" data-id="{{ $item->id }}"
                                                    data-name="{{ $item->name }}" data-status="{{ $item->city_status }}"
                                        ...
                                        @if (in_array('city_list_delete', $permissions))
                                            <a href="#" class="city_delete text-danger w-4 h-4 mr-1"
                                                data-id="{{ $item->id }}">
                                                <svg class="filament-link-icon w-4 h-4 mr-1"
                                                    xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"
                                                    fill="currentColor" aria-hidden="true">
