...
<body style="background: #f4f4f4; font-family: sans, arial; padding: 1em">
    ...
    (isNull($coupon?->discount, 0) + isNull($booking?->payment?->discount, 0))
                ...
                <li>Total paid: <b>Rs.
                        {{ ($booking?->payment?->amount) - (isNull($coupon?->discount, 0) + isNull($booking?->payment?->discount, 0)) }}</b>
                </li>
            </ul>
        </div>
    ...
    (isNull($coupon?->discount, 0) + isNull($booking?->payment?->discount, 0))
                ...
                <li>Pricing plan: <b>{{ $booking->Car->carModel->per_day_km  }} Km/Hr</b></li>

                <li>Total Pricing plan: <b>{{ !empty($booking->Car->carModel->per_day_km) && !empty($price_list['different_hours'])
    ? $booking->Car->carModel->per_day_km * $price_list['different_hours'] : 0 }} Kms</b></li>
                <br>
