                        ...
                        <ul class="navbar-nav navbar-light w-100 text-end text-lg-start">
                            <li class="nav-item ps-0 ps-lg-0 my-auto">
                                <div id="login_button" style="display: {{ Auth::check() ? 'none' : 'block' }};">
                                    <button type="button" class="btn border bg-white text-blue rounded-pill me-1"
                                        id="login_user">Sign-In</button>
                                    <button type="button" class="btn bg-blue text-white rounded-pill"
                                        id="register_user">Sign-Up</button>
                                </div>
                                <div id="after_login_button" class="mt-1 mb-2 nav-link d-flex "
                                    style="display: {{ Auth::check() ? 'block' : 'none !important' }};">
                                    <a href="{{ route('user.profile') }}"
                                        class="bg-white p-2 rounded-pill text-dark text-center my-auto ms-2 text-decoration-none fs-13 fw-600">View
                                        profile</a>
        ...
        <div class="row gy-3 gy-lg-0">
            <div class="col-12 col-lg-6">
                <div class="card p-3 bdr-30 h-100">
                    ...
                    <div class="d-block">
                        <img src="{{ asset('storage/car_image/' . $car_model->carModel->car_image ?? '') }}"
                            alt="car-pic" class="img-fluid">
                    </div>
                    <div class="home-demo">
                    ...
                    <div class="d-flex justify-content-between mt-1">
                        <div class="fs-14 fs-mb-10 fw-500 my-auto {{ $car_model->address ? '' : 'd-none' }}">
                            <img src="{{ asset('user/img/search-result/Group 4 (1).png') }}" alt="location icon"
                    ...
                    <p class="d-lg-flex text-secondary fs-16">
                        <img src="{{ asset('user/img/search-result/iconTransmission.png') }}" alt="icon"
                            class="img-fluid me-2 conf-icon">{{ $car_model->carModel->transmission }}
                        <img src="{{ asset('user/img/search-result/iconSeat.png') }}" alt="icon"
                            class="img-fluid mx-2 conf-icon ms-3"> {{ $car_model->carModel->seat . 'Seats' }}
                        <img src="{{ asset('user/img/search-result/material-symbols-light_air.png') }}" alt="icon"
                            class="img-fluid mx-2 conf-icon ms-3">AC
                        <img src="{{ asset('user/img/search-result/streamline_gas-station-fuel-petroleum.png') }}"
                            alt="icon" class="img-fluid mx-2 conf-icon ms-3"> {{ $car_model->carModel->fuel_type }}
                    </p>
                    <div class=" mt-2">
                        <ul class="fs-6 fw-500 ps-3">
                            <li>Pricing Plan: Total
                                {{ !empty($car_model->carModel->per_day_km) ? ($car_model->carModel->per_day_km * !empty($price_list['different_hours']) ? $price_list['different_hours'] : 0) : $car_model->carModel->per_day_km }}
                                kms, excludes fuel</li>
                ...
                <div class="p-3 bg-blue bdr-30 p-3 h-100">
                    <form>
                        ...
                        <div class="container my-3">
                            <div class="me-0 mx-lg-2 my-auto">
                                ...
                                <div class="d-flex text-white py-2 px-4 date-pick">
                                    <div class="fs-14">
                                        <i class="fa fa-map-marker me-2" aria-hidden="true"></i><span>
                                            {{ array_key_exists(session('city_id'), $city) ? $city[session('city_id')] : '' }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div class="">
                            ...
                            <div class="d-block d-lg-flex justify-content-evenly">
                                ...
                                <div class="text-white my-auto fs-4 d-flex justify-content-center">
                                    <img src="{{ asset('user/img/heroicons_arrows-up-down.svg') }}" alt="icon"
                                        class="img-fluid mx-1 conf-icon ms-3 my-3">
                                </div>
                        ...
                        <div class="d-flex justify-content-between text-white mt-4 border-bottom border-1">
                            ...
                            <p class="fs-14 fs-mb-12 mt-2 mb-3">{{ $price_list['total_days'] ?? 0 }} Day
                                {{ $price_list['total_hours'] ?? 0 }}hr</p>
                        </div>
                        <div
                            class="justify-content-between text-white mt-3 border-bottom border-1 {{ !empty($price_list['festival_amount']) ? 'd-flex' : 'd-none' }}">
                            <p class="fs-14 fs-mb-12 mt-2 mb-3">Total festival fare</p>
                            <p class="fs-14 fs-mb-12 mt-2 mb-3">₹{{ $price_list['festival_amount'] ?? 0 }}</p>
                        </div>
                        <div
                            class="justify-content-between text-white mt-3 border-bottom border-1 {{ !empty($price_list['week_end_amount']) ? 'd-flex' : 'd-none' }}">
                            <p class="fs-14 fs-mb-12 mt-2 mb-3">Weekend fare</p>
                            <p class="fs-14 fs-mb-12 mt-2 mb-3">₹{{ $price_list['week_end_amount'] ?? 0 }}</p>
                        </div>
                        <div
                            class="justify-content-between text-white mt-3 border-bottom border-1 {{ !empty($price_list['week_days_amount']) ? 'd-flex' : 'd-none' }}">
                        ...
                        <div
                            class="d-flex justify-content-between text-white mt-3 mb-1 border-bottom border-1 toggle {{ empty($general_section['show_delivery']) ? 'show' : '' }}">
                        ...
                        @php
                            ...
                            $delivery_fee =
                                empty($general_section['show_delivery']) && !empty($general_section['delivery_fee'])
                                    ? $general_section['delivery_fee']
                            ...
                            $amount =
                                !empty($type) && $type == 2
                                    ? $coupon['discount']
                                    : ($type == 1
                                        ? ($total_price * $coupon['discount']) / 100
                                        : 0);
                            $final_total = $sub_total_price - $amount ?? 0;
                        @endphp
                        <div class="{{ !empty(session('coupon')) ? 'd-flex' : 'd-none' }} justify-content-between text-white mb-2 border-bottom border-1"
                            id="coupon_message">
                            <p class="fs-14 fs-mb-12 mt-2 mb-3">Coupon Amount</p>
                            <p class="fs-14 fs-mb-12 mt-2 mb-3">-
                                <span id="coupon_amount">{{ $amount }}</span> <br><a href="javascript:void(0)"
                                    class="remove-coupon fs-14">Remove</a>
                            </p>
                        ...
                        <div class="d-flex justify-content-between text-white mt-3">
                            ...
                            <div
                                class="input-group booking-inputs-2 my-auto w-100 h-50 border border-white rounded-pill">
                                <input type="text" class="form-control coupon-input" id="coupon_code">
                                <button type="button"
                                    class="input-group-text form-dates bg-light text-blue fs-12 fw-500 py-0 px-3 mx-auto mx-lg-0"
                        ...
                        <div class="mt-1 pt-2 mb-3">
                            <div class="mt-2 mt-lg-5">
                                <input type="hidden" id="final_coupon_amount"
                                    value="{{ session('coupon_amount') }}">
                                <input type="hidden" id="door_delivery"
                                    value="{{ $general_section['delivery_fee'] ?? 0 }}">
                                <input type="hidden" id="final_amount"
                                    value="{{ session('booking_details.total_price') }}">
                                <input type="hidden" id="additional_amount"
                                    value="{{ $delivery_fee + $car_model->carModel->dep_amount ?? 0 }}">
                                <div class="text-white">
                                    <input type="hidden" id="drop_address_pine">
                                    <input type="hidden" id="pickup_address_pine">
                                    <p class="fs-20 fs-mb-16 my-1">
                                        Delivery location</p>
                                    <span id="drop_address"
                                        class="fs-14 fw-500">{{ session('delivery.address') ?? (session('pick-delivery.address') ?? '') }}</span>
                                    <div id="check_delivery_location"
                                        class="{{ !empty(session('delivery.address')) || !empty(session('pick-delivery.address')) ? '' : 'd-none' }}">
                                        <button type="button"
                                            class="input-group-text form-dates fs-16 bg-transparent border-0 font-weight-normal p-0 "
                                            id="delivery_location" style="color: #47ACFF !important;">Location
                                            Change</button>
                                    </div>
                        ...
                        <div class="d-flex justify-content-between flex-column flex-md-row">
                                ...
                                <div class="m-minus-top my-auto">
                                    ...
                                    <div class="mb-3 mb-md-auto toggle">
                                        <button type="button"
                                            ...
                                            <img src="{{ asset('user/img/car-booking/Group.png') }}"
                                                alt="location icons" class="img-fluid d-block me-2"> Select
                                            Drop/Pickup Location
                                        </button>
                                    </div>
                                    <div
                                        class="mb-3 mb-md-auto me-3 {{ !empty($general_section['show_delivery']) ? 'd-none' : '' }}">
                                        <button type="button"
                                            class="btn text-white fs-16 fs-mb-14 fw-500 border-white rounded-pill px-4 d-flex justify-content-center w-100 w-md-auto pickup_location">
                                            <img src="{{ asset('user/img/car-booking/Group.png') }}"
                                                alt="location icons" class="img-fluid d-block me-2"> Select
                            ...
                            <div>
                                <button type="button"
                                    ...
                                    Payment</button>
                                <button type="button"
                                    class="btn bg-white rounded-pill text-blue text-center fs-16 fs-mb-14 px-3 fw-600 w-100 w-md-auto"
                                    style="display: {{ Auth::check() ? 'none' : 'block !important' }};"
                                    id="login_payment">Login To Proceed Payment</button>
                            </div>
                        </div>
                    </form>
                </div>
...
<div class="modal fade m-0 m-md-auto" id="secondModal" tabindex="-1" aria-labelledby="secondModalLabel"
    aria-hidden="true">
            ...
            <div class="modal-body">
                ...
                <div class="p-3">
                    <div id="delivery-section" class="slide-section">
                        ...
                        <input id="delivery-city" type="text" placeholder="Enter Your Delivery Location"
                            class="form-control current_delivery_address">
                    ...
                    <div id="pickup-section" class="slide-section d-none">
                        ...
                        <p class="fs-16 fw-500 mt-2"> Enter manually</p>
                        <input id="custom-city" type="text" placeholder="Enter your return location"
                            class="form-control current_pickup_address">
                        ...
                        <div class="d-flex">
                            <button type="button" class="btn fs-16 my-button mt-4 w-50 w-lg-25"
                                id="conform_address">Conform Location</button>
                            <button type="button" class="btn fs-16 my-current-button rounded-pill mt-4 w-50 w-lg-25"
                                id="pick_current_location">Current Location</button>
                        </div>
                    </div>
                </div>
            </div>
...
<style>
    .pac-container {
        ...
        /* Set it higher than the modal (Bootstrap modals usually have a z-index of 1050) */
    }

    .modal-body {
        position: relative;
        z-index: 1050;
        /* Default z-index for modal content */
    }
</style>
<script async
    src="https://maps.googleapis.com/maps/api/js?key={{ config('services.google_maps.key') }}&libraries=places&callback=initMarker">
</script>
