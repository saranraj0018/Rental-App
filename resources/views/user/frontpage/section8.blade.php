                ...
                @if (!empty($brand_image))
                    @foreach ($brand_image as $key => $image)
                        @if (!empty($image->slug) && $image->slug == 'brand-image')
                            <div class="item">
                                <img src=" {{ !empty($image->name) ? asset('storage/brand-section/' . $image->name) : asset('admin/img/01.png') }}"
                                    alt="car-logo" class="img-fluid">
                            </div>
                        @endif
                    @endforeach
                @else
...
<section>
    <div class="container my-5">
        <div class="d-flex justify-content-between">
            ...
            <div>
                <a href="{{ url('/') }}/user/faq" class="btn text-blue fs-16 fs-mb-12 fw-500">View All <i
